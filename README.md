...
DSH 0.1.6-alpha.2 使用候选版 `dsh-image-viewer@0.1.2-beta.1`。在官方 **插件 → 添加插件** 中只填写该包名与版本，不粘贴整条终端命令；按安装结果提示刷新或重启。下面的 0.1.0 命令面向已支持的旧内核，不代表支持所有新 alpha。
...
插件只读取 DSH 已经渲染的图片 URL，不调用模型或读取供应商凭据。下一版的标注操作会通过 DSH 的附件与输入框接口加入当前草稿，由用户决定是否发送。当前页面内重新打开同一张图片时，区域备注仍会保留；这不等于已发送或永久保存会话。
